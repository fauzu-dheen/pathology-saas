...
def create_share(
    ...
    share = service.create_share(db, user.organization_id, user.id, slide_id, payload.expires_in_hours)
    return share
...
def get_shared_tile(token: str, level: int, col: int, row: int, db: Session = Depends(get_db)):
    ...
    return Response(content=tile_bytes, media_type="image/jpeg", headers={"Cache-Control": "private, max-age=3600"})